...
namespace Service.Utilities
{
    public static class CartMapper
    {
        public static CartReadDTO MapToCartReadDTO(Cart cart)
        {
            ...
            var cartItemsReadDTO =
                cart.CartItems != null
                    ? cart
                        .CartItems.Where(item => item != null)
        ...
        public static CartItemReadDTO MapToCartItemReadDTO(CartItem item)
        {
            ...
            return new CartItemReadDTO
            {
                ...
                Price = isCustom ? item.UnitPrice : item.Product?.Price ?? 0,
                Quantity = item.Quantity,
                ImageUrl = isCustom
                    ? "/images/custom-build.png"
                    : item.Product?.ImageUrl ?? "assets/products/default-product.png",
                Stock = isCustom ? 1 : item.Product?.Stock ?? 0,
        ...
        {
            return new CartItem { ProductId = dto.ProductId, Quantity = dto.Quantity };
        }

        public static CartItemDTO MapToCartItemDTO(CartItem item)
        {
            return new CartItemDTO { ProductId = item.ProductId, Quantity = item.Quantity };
        }
            ...
            return new OrderReadDTO
            {
                ...
                OrderItems =
                    order.OrderItems != null
                        ? order
                            .OrderItems.Where(item => item != null)
