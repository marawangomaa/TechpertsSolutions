...
namespace Repository
{
        ...
        where T : class, IEntity
    {
        ...
        public async Task<IEnumerable<T>> GetAllAsyncIncluded(
            Func<IQueryable<T>, IQueryable<T>> includeBuilder
        )
        {
            var query = includeBuilder != null ? includeBuilder(_dbSet) : _dbSet;
        ...
        )
        {
            var query =
                includeBuilder != null
                    ? includeBuilder(_dbSet.Where(predicate))
        ...
        public async Task<IEnumerable<T>> GetAllWithIncludesAsync(
            Func<IQueryable<T>, IIncludableQueryable<T, object>> includeBuilder
        )
        {
        ...
        private IQueryable<T> ApplyStringIncludes(IQueryable<T> query, string? includeProperties)
        {
            ...
            {
                foreach (
                    var prop in includeProperties.Split(',', StringSplitOptions.RemoveEmptyEntries)
                )
                {
        ...
        private IQueryable<T> ApplyExpressionIncludes(
            IQueryable<T> query,
            params Expression<Func<T, object>>[] includes
        )
        {
